((window) ->
  ...
  @ResourceView = (element, calendar, viewName) ->
    ...
    renderResourceView = (rebuildSkeleton) ->
      ...
      colCnt++  if colCnt is 0
    
      # calc fingerprint
      oldFingerprint = resourceFingerprint
      ids = undefined
      ids = []
      $.each resources, (i, r) ->
        ids.push r.id
      ...
      if dayTable and slotLayer and (rebuildSkeleton or oldFingerprint isnt resourceFingerprint)
        dayTable.remove()
        slotLayer.remove()
        dayTable = false
        slotLayer = false
    ...
    renderResourceOverlay = (col, refreshCoordinateGrid) ->
      coordinateGrid.build()  if refreshCoordinateGrid
      ...
      if ev.which is 1 and opt("selectable") # ev.which==1 means left mouse button
        ...
        hoverListener.start ((cell, origCell) ->
          ...
          if cell and cell.col is origCell.col and not cellIsAllDay(cell)
            ...
            d1tmp = cellDate(origCell)
            d2tmp = cellDate(cell)
            dates1 = [d1tmp, addMinutes(cloneDate(d1tmp), opt("slotMinutes")), d2tmp, addMinutes(cloneDate(d2tmp), opt("slotMinutes"))].sort(cmp)
