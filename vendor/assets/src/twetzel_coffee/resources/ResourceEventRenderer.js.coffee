((window) ->
  @ResourceEventRenderer = ->
    ...
    renderResourceOverlay = @renderResourceOverlay
    ...
    renderEvents = (events, modifiedEventId) ->
      ...
      while i < len
        ...
        i++
      if opt("allDaySlot")
        renderDaySegs compileDaySegs(dayEvents), modifiedEventId, true # PA TODO - Handle next...
    ...
    compileSlotSegs = (events) ->
      ...
      while i < colCnt
        ...
        i++
      segs
    eventsForResource = (resource, events) ->
      resEvents = []
      i = undefined
      i = 0
      while i < events.length
        resEvents.push events[i]  if events[i].resource.id is resource.id  if events[i].resource
        i++
      resEvents
    visEventEndsForResource = (resource, events) ->
      resEvents = []
      i = undefined
      visEventEnds = undefined
      i = 0
      while i < events.length
        resEvents.push events[i]  if events[i].resource.id is resource.id  if events[i].resource
        i++
      ...
      eventElement.draggable
            ...
            if cell
              ...
              unless cell.row
                ...
                if isStart
                  ...
                  revert = true
              revert = revert or (allDay and not columnDelta)
            else
              resetElement()
              revert = true
        ...
        stop: (ev, ui) ->
          ...
          if revert
            ...
            if event.resource
              newColumn = event.resource._col + columnDelta
              resources = calendar.getResources()
    ...
    draggableSlotEvent = (event, eventElement, timeElement) ->
      # TODO - make this work for days + resources?
      ...
      eventElement.draggable
        ...
        stop: (ev, ui) ->
          ...
          clearOverlays()
          trigger "eventDragStop", eventElement, event, ev, ui
          if cell and (columnDelta or minuteDelta or allDay)
            resources = calendar.getResources()
            event.resource = resources[newColumn]
            event.resource._col = newColumn
  ...
  # Duplicated, already defined in "AgendaEventRenderer.js"
  #
