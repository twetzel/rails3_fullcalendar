...
function ResourceView(element, calendar, viewName) {
	...
	t.renderResourcesDaysView = renderResourcesDaysView;
	t.setWidth = setWidth;
	...
	function renderResourcesDaysView(days) {
		updateOptions();
		
		resources = calendar.getResources();
		colCnt = (resources.length + 1) * days;
		dayCnt = days;
	...
	function buildSkeleton() {
		...
		if (dayCnt) {
			...
			for (i=0; i<dayCnt; i++) {
				s +=
					"<th class='fc- fc-day-col" + i + ' ' + headerClass + "' colspan='"+(resources.length+1)+"' />"; // fc- needed for setDayID
			}
			s +=
				"<th class='fc-agenda-gutter " + headerClass + "'>&nbsp;</th>" +
				"</tr>";
		...
		daysHeader = dayHead.find('tr').first();
		resourceHeader = dayHead.find('tr').last();
		dayHeadCells = daysHeader.find('th').slice(1, -1);
		dayHeadCellsTwo = resourceHeader.find('th').slice(1, -1);
	...
	function updateDaDayCells(days) {
		var i;
		var headCell;
		var bodyCell;
		var date;
		var today = clearTime(new Date());
		for (i=0; i<colCnt; i++) {
			date = colDate( parseInt(i / (resources.length + 1)) ); 	// PA massive hack of existing code, but this needs to be changed to support working hours anyway!
			headCell = dayHeadCells.eq(i);
			if(resources.length > 0) {
				if (i >= resources.length) {
					var xyz = (i % (resources.length + 1));
					if (xyz == resources.length) {
						headCell.html(' - - ');
					} else {
						var da_res = resources[xyz].name.split(' ');
						headCell.html(da_res[0].substr(0,1).toUpperCase() + '.' + da_res[1].substr(0,1).toUpperCase() + '.');
					}
				} else {
					var da_res = resources[i].name.split(' ');
					headCell.html(da_res[0].substr(0,1).toUpperCase() + '.' + da_res[1].substr(0,1).toUpperCase() + '.');
				}
			}
			else {
		...
		for (i=0; i<dayCnt; i++) {
			date = colDate(i); 	// PA massive hack of existing code, but this needs to be changed to support working hours anyway!
			headCell = dayHeadCellsTwo.eq(i);
			if(resources.length > 0) {
