...
function ResourceEventRenderer() {
	...
	var renderResourceOverlay = t.renderResourceOverlay;
	...
	function compileSlotSegs(events) {
		...
		for (i=0; i<colCnt; i++) {
			resourceEvents = eventsForResource(resources[ i % (resources.length + 1) ], events);
	...
	function eventsForResource(resource, events) {
		...
		if( typeof resource !== "undefined" && resource !== null ) {
			...
			for(i=0; i<events.length; i++) {
				if( typeof events[i].resource !== "undefined" && events[i].resource !== null ) {
					if ( typeof events[i].resource.id !== "undefined" && events[i].resource.id !== null ){
			...
			for(i=0; i<events.length; i++) {
				if (typeof events[i].resource == "undefined" || events[i].resource == null || typeof events[i].resource.id == "undefined") {
					resEvents.push(events[i]);
				}
			}
			...
			start: function(ev, ui) {
				...
				hoverListener.start(function(cell, origCell, rowDelta, colDelta) {
					...
					if (cell) {
						...
						revert = revert || (allDay && !columnDelta);
					}else{
						resetElement();
